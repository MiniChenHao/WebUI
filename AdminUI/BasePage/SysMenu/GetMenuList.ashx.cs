    ...
    public class GetMenuList : IHttpHandler, IRequiresSessionState
    {
        ...
        public void GetJsonString(List<SysMenuModel> list, string ParentID)
        {
            foreach (SysMenuModel item in GetRowData(list, ParentID))
            {
                if (item.ParentID == "0")
                {
                    MenuJson += "{\"MenuID\":\"" + item.MenuID + "\",\"MenuName\":\"" + item.MenuName + "\",\"MenuImg\":\"" + "<img src='/Theme/Image/32/" + (string.IsNullOrEmpty(item.MenuImg) ? "5005_flag.png" : item.MenuImg) + "' width='16' height='16' />" + "\",\"SortCode\":\"" + item.SortCode + "\",\"NavigateUrl\":\"" + item.NavigateUrl + "\",\"MenuType\":\"" + EnumHelper.GetDescription(typeof(SysEnum.MenuType), item.MenuType) + "\"},";
                    GetJsonString(list, item.MenuID.ToString());
                }
                else if (item.ParentID == ParentID)
                {
                    MenuJson += "{\"MenuID\":\"" + item.MenuID + "\",\"MenuName\":\"" + item.MenuName + "\",\"MenuImg\":\"" + "<img src='/Theme/Image/32/" + (string.IsNullOrEmpty(item.MenuImg) ? "5005_flag.png" : item.MenuImg) + "' width='16' height='16' />" + "\",\"SortCode\":\"" + item.SortCode + "\",\"NavigateUrl\":\"" + item.NavigateUrl + "\",\"MenuType\":\"" + EnumHelper.GetDescription(typeof(SysEnum.MenuType), item.MenuType) + "\",\"_parentId\":\"" + item.ParentID + "\"},";
                    GetJsonString(list, item.MenuID.ToString());
                }
            }
        }
