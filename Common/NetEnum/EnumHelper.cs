...
namespace Common.NetEnum
{
    ...
    public class EnumHelper
    {
        ...
        /// <param name="member">枚举成员的常量名或常量值,范例:Enum1枚举有两个成员A=0,B=1,则传入"A"或"0"获取 Enum1.A 枚举类型</param>
        ...
        /// 获取枚举值转换为列表
        /// </summary>
        /// <param name="enumType">（参数）Type enumType 枚举</param>
        /// <returns></returns>
        public static List<ListItem> GetEnumList(Type enumType)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                if (!string.IsNullOrEmpty(Value))
                {
                    ...
                    ListItem lista = new ListItem(text, Value);
                    list.Add(lista);
                }
            }
            return list.ToList();
        ...
        /// </summary>
        /// <param name="enumType">（参数）Type enumType 枚举</param>
        /// <param name="value">（参数） object value 枚举值</param>
        /// <returns>String描述</returns>
        public static string GetEnumDescriptionByValue(Type enumType, object value)
        {
            string result = string.Empty;

            string enumvalue = System.Enum.GetName(enumType, value);
            if (!string.IsNullOrEmpty(enumvalue))
            {
                FieldInfo fieldinfo = enumType.GetField(enumvalue);
